...
`defineConfig` is an identity function - it exists purely so the object is checked against `ConfigFile` where you write it, giving you autocompletion and catching typos inline. Annotating a variable works exactly as well if you prefer it:
