...
A TypeScript config file needs no extra tooling on Node.js versions that strip TypeScript types natively - the CLI loads it directly. On older versions, install either `esbuild-runner` (alongside `esbuild`) or `ts-node`.

Node's built-in support only _erases_ type annotations. If your config relies on syntax that needs a real transform - `enum`, `namespace`, parameter properties - install one of those transpilers; the CLI prefers them whenever they are present.
...
// on Node.js versions that strip types natively, plain `node` is enough;
// otherwise use tsx, esbuild-runner (esr) or ts-node
exec(`node ${cliPath} config.ts`)
```
