...
export interface DevToolsEnhancerOptions {
  ...
  /**
   * Customizes how actions and state are serialized and deserialized. Can be a
   * boolean or object. If given a boolean, the behavior is the same as if you
   * were to pass an object and specify `options` as a boolean. Giving an object
   * allows fine-grained customization using the `replacer` and `reviver`
   * functions.
   */
  serialize?:
    | boolean
    | {
         ...
         * - object, which contains `date`, `regex`, `undefined`, `error`, `symbol`, `map`, `set` and `function` keys. For each of them you can indicate if to include (by setting as `true`). For `function` key you can also specify a custom function which handles serialization. See {@link https://github.com/kolodny/jsan | jsan} for more details.
         */
        options?:
          | undefined
          | boolean
        ...
        /**
         * {@link https://developer.mozilla.org/en/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify#The_replacer_parameter | JSON replacer function}
         * used for both actions and states stringify. In addition, you can
         * specify a data type by adding a
         * {@link https://github.com/zalmoxisus/remotedev-serialize/blob/master/helpers/index.js#L4 | __serializedType__}
         * key. So you can deserialize it back while importing or persisting
         * data. Moreover, it will also
         * {@link https://cloud.githubusercontent.com/assets/7957859/21814330/a17d556a-d761-11e6-85ef-159dd12f36c5.png | show a nice preview showing the provided custom type}.
         */
        replacer?: (key: string, value: unknown) => any

        /**
         * {@link https://developer.mozilla.org/en/docs/Web/JavaScript/Reference/Global_Objects/JSON/parse#Using_the_reviver_parameter | JSON reviver function}
         * used for parsing the imported actions and states. See
         * {@link https://github.com/zalmoxisus/remotedev-serialize/blob/master/immutable/serialize.js#L8-L41 | remotedev-serialize}
         * as an example on how to serialize special data types and get them
         * back.
         */
        reviver?: (key: string, value: unknown) => any

        /**
         * Automatically serialize/deserialize immutablejs via
         * {@link https://github.com/zalmoxisus/remotedev-serialize | remotedev-serialize}.
         * Just pass the Immutable library. It will support all ImmutableJS
         * structures. You can even export them into a file and get them back.
         * The only exception is `Record` class, for which you should pass this
         * in addition the references to your classes in `refs`.
         */
        immutable?: any

        /**
         * ImmutableJS `Record` classes used to make possible restore its
         * instances back when importing, persisting...
         */
        refs?: any
      }
  /**
  ...
  /**
   * Set to `true` or a stacktrace-returning function to record call stack
   * traces for dispatched actions.
   *
   * @default false
   */
